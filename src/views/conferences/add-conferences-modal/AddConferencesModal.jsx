...
const AddConferencesModal = ({ open }) => {
    const [value, setValue] = useState('1');
    const dispatch = useDispatch();

    const handleClose = () => dispatch(modalOpenConferencias(!open));
                    ...
                    <TabContext value={value}>
                        <Box sx={{ borderBottom: 1, borderColor: 'divider' }}>
                            <TabList onChange={handleChange}>
                                <Tab label="Nueva Conferencia" sx={{ textTransform: 'uppercase', width: '20%' }} value="1" />
                            </TabList>
                        </Box>
                        <TabPanel value="1">
                            <TabsConference />
