...
const AddAnnexUser = ({ open }) => {
    const [value, setValue] = useState('1');
    const dispatch = useDispatch();

    const handleClose = () => dispatch(modalOpenUser(!open));
                    ...
                    <TabContext value={value}>
                        <Box sx={{ borderBottom: 1, borderColor: 'divider' }}>
                            <TabList onChange={handleChange}>
                                <Tab label="Usuario" sx={{ textTransform: 'uppercase', width: '20%' }} value="1" />
                            </TabList>
                        </Box>
                        <TabPanel value="1">
                            <User />
...
AddAnnexUser.propTypes = {
    open: PropTypes.bool
};

export default AddAnnexUser;
